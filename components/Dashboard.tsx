...
import { AppState, GenerationMode, GeneratedImage, EditorTab, ExportFormat, EntityProfile, CreativeConcept } from '../types';
...
import { Promptor } from './generation/Promptor';
import { IdeationAssistant } from './generation/IdeationAssistant';
import { ArrowRight, Sparkles, User, Package, Download, ThumbsUp, ThumbsDown, CheckCircle2, Circle, AlertTriangle, Layers, Maximize2, X, Palette, Wand2, SplitSquareHorizontal, Crop, Check, Wrench, Play, Square, Clock, Zap, Star, AlertCircle, Settings, ChevronDown, Camera, MapPin, ImagePlus, Trash2, Lightbulb } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ appState, onImageGenerated, onFeedback, onQuickAI }) => {
  ...
  const isUltraRealistic = true; // Always ultra-realistic for best quality
  const [isPackshot, setIsPackshot] = useState(false);
  ...
  // === CREATION MODE ===
  const [creationMode, setCreationMode] = useState<'manual' | 'assistant'>('manual');
  ...
  const handleConceptSelection = (concept: CreativeConcept) => {
    setPrompt(concept.suggestedPrompt);
    // Logic to select assets if IDs match known assets
    if (concept.suggestedAssets) {
      if (concept.suggestedAssets.personId) setSelectedPersonIds([concept.suggestedAssets.personId]);
      if (concept.suggestedAssets.productId) setSelectedProductIds([concept.suggestedAssets.productId]);
      if (concept.suggestedAssets.locationId) setSelectedLocationId(concept.suggestedAssets.locationId);
    }
    setCreationMode('manual');
    showNotification('success', 'Concept appliqué !');
  };
  ...
  const handleInspirationUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const remainingSlots = 3 - inspirationImages.length;
  ...
  const handleGenerate = async () => {
    ...
    // Simplified: always ultra-realistic, no persona or style modifiers
    const finalPrompt = prompt;

    // Get selected location
    const selectedLocation = appState.locations?.find(l => l.id === selectedLocationId);
    ...
    const generateOne = async (): Promise<string> => {
      return generateBrandVisual(
        ...
        {
          injectPulseeBranding: false,
          prioritizeProductFidelity: true,
          ultraRealistic: true,
          isPackshot: isPackshot,
      ...
      onItemComplete: (item) => {
        if (item.status === 'completed' && item.result) {
          const newImage: GeneratedImage = {
            ...
            styleId: 'natural', timestamp: Date.now(),
    ...
    <div className="container mx-auto px-4 py-4 sm:py-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className={`lg:col-span-1 space-y-6 ${showMobileFilters ? 'block' : 'hidden lg:block'}`}>
          ...
          <div className="bg-gray-900 border border-gray-800 p-1 rounded-xl flex">
            <button onClick={() => setCreationMode('manual')} className={`flex-1 py-3 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${creationMode === 'manual' ? 'bg-cyan-600/20 text-cyan-400 shadow-sm border border-cyan-500/30' : 'text-gray-500 hover:text-white'}`}>
              <Zap size={14} /> Manuel
            </button>
            <button onClick={() => setCreationMode('assistant')} className={`flex-1 py-3 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${creationMode === 'assistant' ? 'bg-purple-600/20 text-purple-400 shadow-sm border border-purple-500/30' : 'text-gray-500 hover:text-white'}`}>
              <Lightbulb size={14} /> Idéation
            </button>
          </div>

          {creationMode === 'assistant' ? (
            <IdeationAssistant
              onConceptSelected={handleConceptSelection}
            ...
            <>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-white font-bold"><ImagePlus size={16} className="text-cyan-500" /><h3>Inspiration</h3></div>
                <div className="grid grid-cols-3 gap-2">
                  {inspirationImages.map((img, idx) => (
                    <div key={idx} className="relative aspect-square rounded-lg overflow-hidden border border-gray-700 group">
                      ...
                      <button onClick={() => setInspirationImages(prev => prev.filter((_, i) => i !== idx))}
                  ...
                  {inspirationImages.length < 3 && (
                    <label className="aspect-square rounded-lg border-2 border-dashed border-gray-700 hover:border-cyan-500 hover:bg-cyan-500/10 transition-colors cursor-pointer flex flex-col items-center justify-center gap-2 text-gray-500 hover:text-cyan-400">
                      <ImagePlus size={20} />
                      <span className="text-[10px] uppercase font-bold">Ajouter</span>
                      <input type="file" accept="image/*" multiple onChange={handleInspirationUpload} className="hidden" />
                    </label>
                  )}
                </div>

                {inspirationImages.length > 0 && <p className="text-[10px] text-gray-500">Ces images guideront la composition.</p>}
              </div>
